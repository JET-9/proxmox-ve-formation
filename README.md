# proxmox-ve-formation
Support de formation complet sur **Proxmox VE**, destiné à des étudiants de niveau **Master (M1)** et à des profils techniques souhaitant comprendre les enjeux réels de la virtualisation en production.

## Contenu
- Fondamentaux de la virtualisation
- Proxmox VE : VM, LXC, réseau, stockage
- Clustering, quorum et QDevice
- Haute disponibilité
- Sécurité, exploitation et bonnes pratiques
- Projet de fin de formation (étude de cas professionnelle)

## Licence
© Ilyes Hamdane – CC BY-NC-ND 4.0  

Ce contenu est mis à disposition à des fins **non commerciales uniquement**.  
Toute utilisation académique institutionnelle, professionnelle ou commerciale est soumise à licence.

Voir le fichier [LICENSE.md](LICENSE.md) pour plus de détails.

## Contact sur linkedin
Pour toute demande ou collaboration :  
**Ilyes Hamdane**

## Accès aux supports de formation

Les supports complets de la formation (slides et projet de fin de formation) sont disponibles via le lien ci-dessous :

https://drive.google.com/drive/folders/1SNuM_r7T47FILIhZLj4b9Cj6FiOcI-Hb?usp=sharing

Contenu diffusé sous licence **CC BY-NC-ND 4.0**.
Toute utilisation académique institutionnelle, professionnelle ou commerciale est soumise à licence.
